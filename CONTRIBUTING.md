...
sudo ./venv/bin/python3 -m ctx 
sudo ./venv/bin/python3 -m ctx <optional params>
sudo ./venv/bin/python3 -m ctx -c 44 -s "dev" -i wlan2 --no11r --logging debug
```
...
1. Format your code with `tox -e format` (this will run autoflake with desired options, black, and isort on the ctx codebase)
